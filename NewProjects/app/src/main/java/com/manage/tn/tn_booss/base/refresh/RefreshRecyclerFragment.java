package com.manage.tn.tn_booss.base.refresh;
...
import com.manage.tn.tn_booss.databinding.FragmentRefreshRecyclerBinding;
...
public abstract class RefreshRecyclerFragment extends
        BaseFragment<FragmentRefreshRecyclerBinding> {
    // 请求状态 - 下拉刷新 还是 加载更多
    ...
    * 获取网络返回数据后执行UI和数据更新
    * */
    public  void afterReturnDataForUi(String uuid,BaseResponse event) {
    ...
    protected void onLoadMore(BaseResponse event) {
        if(event.getData() instanceof List){
            if (((List)event.getData()).size() < pageCount) {
     ...
     * @param adapter 适配器
     */
    public abstract void initDataAdapter(HeaderFooterAdapter adapter);
